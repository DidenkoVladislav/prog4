...
{
    FILE *fp = (mode == TEXT_MODE) ? (fopen(filename, "w")) : (fopen(filename, "wb"));
    if (fp == NULL)
    {
...
{
    FILE *fp = (mode == TEXT_MODE) ? (fopen(filename, "r")) : (fopen(filename, "rb"));
    if (fp == NULL)
    {
    ...
    Matrix2d *m = (mode == TEXT_MODE) ? (load_matrix2d_from_text(fp)) : (load_matrix2d_from_binary(fp));
...
int get_matrix2d_from_binary_file(const char *filename, int index, Matrix2d *result)
{
    if (index < 0)
    {
        printf("Ошибка индекса %d: ожидалось index >= 0\n", index);
    ...
    for (int i = 0; i < index; i++)
    {
        ...
        size_t elements_size = ((type == union_var_i) ? (sizeof(int)) : (sizeof(double))) * rows * cols;
